...
public class AnswerContentNotificationTest {
    @Mock
    ...
    @InjectMocks
    private AnswerContentNotification target;

    @Test
    public void supports_shouldReturnExpectedValue() {
        when(event.isAnswer()).thenReturn(true, false);
    ...
    public void shouldDisplayInConfiguration_shouldReturnExpectedValueWhenModuleIsEnabled() {
        when(pluginAccessor.isPluginModuleEnabled(QuestionType.ANSWER.pluginModuleKey())).thenReturn(true);
        assertThat(target.shouldDisplayInConfiguration(), is(true));
    }

    @Test
    public void getActivityKey_shouldReturnExpectedValue() {
        assertThat(target.getActivityKey(event), is(Optional.of("slack.activity.question-answered")));
    }
