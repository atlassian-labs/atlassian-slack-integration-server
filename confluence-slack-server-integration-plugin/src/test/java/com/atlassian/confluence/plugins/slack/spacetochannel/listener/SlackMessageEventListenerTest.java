...
public class SlackMessageEventListenerTest {
    ...
    public void slashCommand_shouldRespondWithSearchResults() throws InvalidSearchException {
        ...
        when(slackSlashCommand.getResponseUrl()).thenReturn("ru");
        when(slackSlashCommand.getSlackLink()).thenReturn(slackLink);
        when(searchManager.search(searchCaptor.capture())).thenReturn(searchResults);
        ...
        when(searchResults.isLastPage()).thenReturn(true);
        ...
        ISearch search = searchCaptor.getValue();
        assertThat(((TextQuery) search.getQuery()).getQuery(), is(query));
        assertThat(search.getStartOffset(), is(0));
        assertThat(search.getLimit(), is(5));
    }
    ...
    public void slashCommand_shouldSearchPagesAndBlogPostsOnly() throws InvalidSearchException {
        ...
        when(searchResults.isLastPage()).thenReturn(false);
        when(attachmentBuilder.buildAttachment(page)).thenReturn(attachment);
        when(attachmentBuilder.searchFooter(query)).thenReturn(footerAttachment);
        when(attachmentBuilder.getSearchResultsTitleMessage(5, query)).thenReturn("search-res");
        when(slackClientProvider.withLink(slackLink)).thenReturn(slackClient);
        CommonTestUtil.bypass(asyncExecutor);
        ...
        assertThat(attachments.size(), is(6));
        assertThat(attachments, contains(attachment, attachment, attachment, attachment, attachment, footerAttachment));
        List<ISearch> searches = searchCaptor.getAllValues();
        IntStream.range(0, searches.size()).forEach(i -> {
            ISearch search = searches.get(i);
            assertThat(((TextQuery) search.getQuery()).getQuery(), is(query));
            assertThat(search.getStartOffset(), is(i * 5));
            assertThat(search.getLimit(), is(5));
        });
    }
