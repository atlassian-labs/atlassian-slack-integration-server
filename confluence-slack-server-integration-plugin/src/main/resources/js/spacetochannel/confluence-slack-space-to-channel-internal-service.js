define('confluence/slack/space-to-channel/internal/service', [
    'underscore',
    'jquery'
],
/**
 * Internal services used to manage space to channel configuration
 *
...
function (
    ...
    var Routes = {
        allChannels: function(space) {
            return AJS.Confluence.getContextPath() + '/rest/slack/latest/channels?teamId=' + getUserAndTeamInfo().teamId
                + '&key=' + (space ? space.key : "");
        },
        channelConfig: function(spaceKey, teamId, channelId, notifName, initialLink) {
            var url = AJS.Confluence.getContextPath() + '/rest/slack/latest/config/' + encodeURI(spaceKey) + '/'
    ...
    var configService = (function() {
        return {
            ...
            toggleNotification: function (spaceKey, teamId, channelId, notifName, enable, initialLink) {
                var requestType = enable ? 'PUT' : 'DELETE';
                return $.ajax(Routes.channelConfig(spaceKey, teamId, channelId, notifName, initialLink), {
                    type: requestType
                });
            }
        };
    ...
    var channelServicePromise = (function(space) {
        var allowMultipleMappingsToSameChannel = space == null;
    ...
    return function (space) {
        ...
        return {
            ...
            channelServicePromise: channelServicePromise(space),
