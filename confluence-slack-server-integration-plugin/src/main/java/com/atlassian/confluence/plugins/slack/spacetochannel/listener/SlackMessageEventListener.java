...
public class SlackMessageEventListener extends AutoSubscribingEventListener {
    ...
    private Pair<List<Attachment>, String> searchPages(final String query, final String slackUserId) {
        ...
        List<SpaceContentEntityObject> foundPagesAndBlogs = user.isPresent()
                ? confluenceUserImpersonator.impersonate(
                        user.get().right(),
                        () -> searchAtMostThatManyPagesOrBlogs(query, maxSearchResultsToShow),
        ...
        List<Attachment> contentAttachments = foundPagesAndBlogs.stream()
                ...
                .collect(Collectors.toList());
        List<Attachment> attachments = new ArrayList<>(contentAttachments);
    ...
    private List<SpaceContentEntityObject> searchAtMostThatManyPagesOrBlogs(final String query,
        ...
        List<SpaceContentEntityObject> pagesOrBlogs = new ArrayList<>();
        try {
            ...
            do {
                SearchFilter filter = ContentPermissionsSearchFilter.getInstance()
                        .and(new ArchivedSpacesSearchFilter(false, spaceManager))
                        .and(new SpacePermissionsSearchFilter(null, null));
                ISearch searchConfig = new ContentSearch(textQuery, sort, filter, offset, pagesOrBlogsToSearch);
                searchResults = searchManager.search(searchConfig);
            List<Searchable> entities = searchManager.convertToEntities(searchResults, EntityVersionPolicy.LATEST_VERSION);
                List<SpaceContentEntityObject> spaceContentEntities = entities.stream()
                        .filter(item -> item instanceof SpaceContentEntityObject)
                        .map(item -> (SpaceContentEntityObject) item)
                        .collect(Collectors.toList());
                pagesOrBlogs.addAll(spaceContentEntities);
                offset += pagesOrBlogsToSearch;
            } while (pagesOrBlogs.size() < pagesOrBlogsToSearch && !searchResults.isLastPage());

        } catch (InvalidSearchException e) {
            log.error("Error during pages search", e);
        }

        return pagesOrBlogs.subList(0, Math.min(pagesOrBlogs.size(), pagesOrBlogsToSearch));
    }
}
