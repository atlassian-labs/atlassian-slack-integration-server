...
public class AnswerContentNotification extends BaseAbstractPageNotification<QuestionEvent> {
    private final PluginAccessor pluginAccessor;

    public AnswerContentNotification(final AttachmentBuilder contentCardBuilder,
    ...
    public boolean shouldDisplayInConfiguration() {
        try {
            return pluginAccessor.isPluginModuleEnabled(QuestionType.ANSWER.pluginModuleKey());
        } catch (Throwable e) {
            return false;
        }
    }
    ...
    protected Optional<String> getActivityKey(final QuestionEvent event) {
        return Optional.of("slack.activity.question-answered");
    }
