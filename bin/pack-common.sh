...
(
    ...
    atlas-mvn install --batch-mode -Dmaven.test.skip=true \
        "$@" \
        -pl com.atlassian.plugins:atlassian-slack-server-integration-parent,\
...
slack-server-integration-test-common && \
    ...
    rm -rf confluence-slack-integration/confluence-slack-server-integration-plugin/target/classes/com/atlassian/plugins/slack && \
    rm -f confluence-slack-integration/confluence-slack-server-integration-plugin/target/dependency-maven-plugin-markers/com.atlassian.plugins-slack-server-integration-common-jar-* && \
